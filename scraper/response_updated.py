...
def history(month,year):
    ...
    with open(f'{month_year}.html', 'r') as file_obj:
        data = file_obj.read()
    ...
    with open(f"{month_year}.txt", "w") as file:
...
for i in years:    
    for j in months:        
        ...
        mm_yy=j+'_'+i
        print(mm_yy)
        req1 = Request('https://en.wikipedia.org/wiki/Responses_to_the_COVID-19_pandemic_in_'+mm_yy)
        try:
