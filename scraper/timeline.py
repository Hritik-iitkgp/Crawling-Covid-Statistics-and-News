...
def history(year,month=''):
    ...
    if month!='':
        ...
        filenew=month_year
        with open(f'{filenew}.html', 'r') as file_obj:
            data = file_obj.read()
    ...
    with open(f"{filenew}.txt", "w") as file:
...
for i in years:    
    for j in months:        
        ...
        try:
            ...
            with open(f'{mm_yy}.html', 'w', encoding="utf-8") as f2:
                f2.write(data)
                f2.close
                data1=[]
            # history(j,i)
...
for i in yearsleft:   
    ...
    print(i)
    req1 = Request('https://en.wikipedia.org/wiki/Timeline_of_the_COVID-19_pandemic_in_'+i)
    try:
        webpage1 = urlopen(req1).read()
        data = webpage1.decode("utf8")
        with open(f'{i}.html', 'w', encoding="utf-8") as f2:
            f2.write(data)
            f2.close
            data1=[]
        history(i)
